...
import { archiveProductVariantAction } from "./actions";

export default async function ProductsPage() {
  const session = await requireSession();
  const t = await getTranslations("catalog");
  const tCommon = await getTranslations("common");
  const variants = await listArticleVariants(session.organizationId, "PRODUCT");
          ...
          <tbody>
            ...
            {variants.map((v) => (
              <tr key={v.id} className="border-t border-neutral-100">
                ...
                <td className="px-4 py-2">{v.article.category ?? "—"}</td>
                ...
                <td className="px-4 py-2 text-end">
                  {v.status === "ACTIVE" && (
                    <ArchiveButton id={v.id} action={archiveProductVariantAction} />
                  )}
                </td>
              </tr>
            ))}
